...
export const Cart = () => {
  ...
  //Calculating the total amount in each cart item getted added or removed.
  useEffect(() => {
    setTotalAmount(cart.reduce((acc, curr) => acc + curr.price, 0));
  }, [cart]);

  return (
    /*If the cartItem has length more that zero then just the items at one end of page
    and on the other end show the price summary.
    whereas if there is no item in cart show Empty and button to redirect to home page. */
    <div >
      {cart.length > 0 ? (
